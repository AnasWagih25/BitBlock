...
import { getAutoMLPreset } from "../ml/edgeAutoPreset";
...
import CodeEditorView from "../components/ui/CodeEditorView";
...
export default function IDEPage() {
  ...
  const [viewMode, setViewMode] = useState<"blocks" | "ml" | "code">("blocks");
  // ── Code Editor state ────────────────────────────────────────────────────
  const DEFAULT_CODE = `// Generated by BitBlock — write Arduino C/C++ code here
...
void loop() {
  ...
  const [codeEditorContent, setCodeEditorContent] = useState(DEFAULT_CODE);
  const [syncEnabled, setSyncEnabled] = useState(false);
  const [codeOverridesBlocks, setCodeOverridesBlocks] = useState(false);
  ...
  }, [user, loading, projectId, navigate, setProject]);
  ...
  useEffect(() => {
    ...
    const firstTask = currentBoard.supportedMLTasks[0] as MLTask;
    setMlTask(firstTask);
    
    const preset = getAutoMLPreset(firstTask, currentBoard);
    if (preset && preset.architecture) {
      setMlArch(preset.architecture);
    } else {
      const archs = TASK_ARCHITECTURES[firstTask] || [];
      setMlArch(archs.length > 0 ? archs[0] : "");
    }
  }, [currentBoard.id, currentBoard.mlSupport]);
  ...
  const generateCode = useCallback(() => {
    ...
    try {
      ...
      if (gen) {
        ...
        const { errors } = validateWorkspace(workspaceRef.current);
        if (errors.length > 0) {
          setCompileStatus("error");
          setCompileMessage(errors.join("\n"));
          setGeneratedCode("// Error: " + errors[0]);
          return;
        }
        ...
        // Sync to editor if applicable
        if (syncEnabledRef.current && !codeOverridesBlocksRef.current) {
          if (wrapped && wrapped.trim()) {
            setCodeEditorContent(wrapped);
          }
        }
  ...
  useEffect(() => {
    const handleMiddleDown = (e: PointerEvent) => {
      if (e.button === 1 && blocklyDivRef.current?.contains(e.target as Node)) {
        e.preventDefault();
        e.stopPropagation();
        ...
        const initialScrollX = typeof ws.scrollX === 'number' ? ws.scrollX : 0;
        const initialScrollY = typeof ws.scrollY === 'number' ? ws.scrollY : 0;
        ...
        const onPointerMove = (moveEvent: PointerEvent) => {
          ...
          ws.scroll(initialScrollX + dx, initialScrollY + dy);
        };
    ...
    window.addEventListener('pointerdown', handleMiddleDown, { capture: true });
    return () => {
      window.removeEventListener('pointerdown', handleMiddleDown, { capture: true } as any);
    };
  }, []);

  // Prevent browser UI zooming (Ctrl+Wheel or pinch-to-zoom) while allowing Blockly zoom
  useEffect(() => {
    ...
    return () => {
      ...
      document.removeEventListener('touchmove', handleTouch, { passive: false } as any);
    };
  }, []);

  useEffect(() => {
    // Whenever the code changes, invalidate the previously compiled firmware
    setFirmwareReady(false);
    setFirmwareBinary(null);
    setCompileStatus(s => s === "success" ? "idle" : s);
  }, [codeEditorContent]);
  ...
  // Stable refs for use inside Blockly change listener closure
  const syncEnabledRef = useRef(false);
  const codeOverridesBlocksRef = useRef(false);
  useEffect(() => { syncEnabledRef.current = syncEnabled; }, [syncEnabled]);
  useEffect(() => { codeOverridesBlocksRef.current = codeOverridesBlocks; }, [codeOverridesBlocks]);

  const handleToggleSync = () => {
    setSyncEnabled((prev) => {
      const next = !prev;
      if (next) {
        // Turning sync ON → immediately pull current blockly code into editor
        setCodeOverridesBlocks(false);
  ...
  const handleCompile = async () => {
    ...
    // If in Code Editor view, use the editor's content directly
    let codeToCompile: string;
    if (viewMode === "code") {
      codeToCompile = codeEditorContent;
    } else {
      codeToCompile = generatedCode;
      try {
        if (workspaceRef.current && Blockly) {
          const gen = Blockly.javascriptGenerator || Blockly.JavaScript;
          if (gen) {
    ...
    if (!/void\s+setup\s*\(/.test(codeToCompile) || !/void\s+loop\s*\(/.test(codeToCompile)) {
      await alert("Compile blocked: code is missing setup() or loop() functions. Add them and try again.");
      return;
    }
    ...
    try {
      ...
      // Extract any custom GitHub library URLs embedded in the code by the Library Manager
      const customLibUrls = Array.from(new Set(
        (codeToCompile.match(/\/\/\s*@bitblock-lib\s+(https:\/\/github\.com\/\S+)/g) || [])
          .map(match => match.split('@bitblock-lib')[1].trim())
      ));
  ...
  const loadExample = async (example: typeof MARKETPLACE_EXAMPLES[0]) => {
    ...
    try {
      ...
      setTimeout(() => generateCodeRef.current(), 50);
  ...
  return (
    <div className="ide-layout" style={{
      ...
      gridTemplateColumns: (viewMode === "ml" || viewMode === "code") ? "1fr" : (sidebarExpanded ? "260px 1fr 340px" : "48px 1fr 340px"),
      gridTemplateRows: "60px 1fr 28px",
      transition: "grid-template-columns 0.3s cubic-bezier(0.4, 0, 0.2, 1)"
    }}>

      {/* ── Navbar ─────────────────────────────────────── */}
      <header className="ide-toolbar" style={{
        display: "flex", alignItems: "center", gap: 0,
        padding: "0",
        borderBottom: "1px solid rgba(157,39,222,0.18)",
        background: "rgba(8,4,18,0.97)",
        backdropFilter: "blur(20px)",
        boxShadow: "0 1px 0 rgba(157,39,222,0.1), 0 4px 24px rgba(0,0,0,0.4)",
        position: "relative",
        zIndex: 100,
      }}>

        {/* ── Left: Logo + Project name ── */}
        <div style={{ display: "flex", alignItems: "center", gap: 0, paddingLeft: 16, paddingRight: 16, borderRight: "1px solid rgba(157,39,222,0.12)", height: "100%" }}>
          <Link to="/dashboard"
          ...
          >
            <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="rgba(157,39,222,0.6)" strokeWidth="2.5">
              <path d="M19 12H5M12 19l-7-7 7-7" />
            </svg>
            <span style={{ fontFamily: "Superstar, fantasy", fontSize: 20, color: "#9D27DE", letterSpacing: "0.02em", lineHeight: 1 }}>BB</span>
          </Link>

          <div style={{ width: 1, height: 20, background: "rgba(157,39,222,0.15)", margin: "0 12px" }} />

          <div style={{ display: "flex", flexDirection: "column", gap: 1 }}>
            <span style={{ fontSize: 13, fontWeight: 700, color: "#F2F2F0", letterSpacing: "-0.01em", lineHeight: 1.1 }}>
              {project?.name}
            </span>
            <span style={{ fontSize: 10, color: "rgba(242,242,240,0.28)", letterSpacing: "0.05em" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", height: "100%", padding: "0 8px" }}>
          {([
            ...
            },
            {
              ...
              onClick: () => {
                setViewMode("code");
                if (!codeOverridesBlocks && workspaceRef.current && Blockly) {
                  try {
                    const gen = Blockly.javascriptGenerator || Blockly.JavaScript;
                    if (gen) {
                      const boardId = project?.board || "esp32-wroom";
                      ...
                      const rawCode = gen.workspaceToCode(workspaceRef.current);
                      const wrapped = compiler.assemble(rawCode);
                      if (wrapped && wrapped.trim()) setCodeEditorContent(wrapped);
                    }
                  } catch { /* ignore */ }
                }
              },
            },
            {
              ...
              title: !isMlSupported ? "Board does not support ML Pipeline" : undefined,
            },
          ] as any[]).map((tab) => {
            ...
            return (
              <button
                ...
                style={{
                  ...
                  border: "none",
                  borderBottom: isActive ? "2px solid #9D27DE" : "2px solid transparent",
                  borderTop: "2px solid transparent",
                  color: isActive ? "#F2F2F0" : tab.disabled ? "rgba(242,242,240,0.2)" : "rgba(242,242,240,0.5)",
                  fontSize: 12,
                ...
                onMouseEnter={e => { if (!tab.disabled && !isActive) e.currentTarget.style.color = "rgba(242,242,240,0.8)"; }}
                onMouseLeave={e => { if (!tab.disabled && !isActive) e.currentTarget.style.color = "rgba(242,242,240,0.5)"; }}
              >
                {tab.icon}
                {tab.label}
                {tab.id === "ml" && !isMlSupported && (
                  <span style={{ fontSize: 9, padding: "1px 5px", borderRadius: 3, background: "rgba(239,68,68,0.12)", color: "#f87171", fontWeight: 600, letterSpacing: "0.05em" }}>N/A</span>
                )}
              </button>
            );
          })}
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 4, paddingRight: 16, paddingLeft: 8 }}>

          {/* Board selector group */}
          <div ref={boardDropdownRef} style={{ position: "relative", display: "flex", alignItems: "center", gap: 8, padding: "0 12px", borderRight: "1px solid rgba(157,39,222,0.12)", marginRight: 8, height: 36 }}>
            <div style={{ width: 7, height: 7, borderRadius: "50%", background: currentBoard.color, boxShadow: `0 0 6px ${currentBoard.color}`, flexShrink: 0 }} />
            <button
              onClick={() => setShowBoardDropdown(!showBoardDropdown)}
              style={{
                background: showBoardDropdown ? "rgba(157,39,222,0.2)" : "rgba(157,39,222,0.08)",
                border: showBoardDropdown ? "1px solid rgba(157,39,222,0.4)" : "1px solid rgba(157,39,222,0.2)",
                color: "#F2F2F0", borderRadius: 6, padding: "4px 8px", fontSize: 11,
                cursor: "pointer", outline: "none", fontFamily: "Space Grotesk, sans-serif",
                display: "flex", alignItems: "center", gap: 6,
                transition: "all 0.15s"
              }}
              onMouseEnter={e => {
                if (!showBoardDropdown) {
                  e.currentTarget.style.background = "rgba(157,39,222,0.15)";
                  e.currentTarget.style.borderColor = "rgba(157,39,222,0.3)";
                }
              }}
              onMouseLeave={e => {
                if (!showBoardDropdown) {
                  e.currentTarget.style.background = "rgba(157,39,222,0.08)";
                  e.currentTarget.style.borderColor = "rgba(157,39,222,0.2)";
                }
              }}
            >
              <span style={{ fontWeight: 600 }}>{currentBoard.name}</span>
              <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" style={{ transform: showBoardDropdown ? "rotate(180deg)" : "rotate(0deg)", transition: "transform 0.2s" }}>
                <path d="M6 9l6 6 6-6"/>
              </svg>
            </button>
            ...
            {showBoardDropdown && (
              <div style={{
                position: "absolute",
                top: "calc(100% + 4px)",
                right: 12, // Align to the right edge of the padding
                minWidth: 150,
                width: "max-content",
                ...
                gap: 2,
                boxShadow: "0 12px 30px rgba(0,0,0,0.6), 0 0 40px rgba(157,39,222,0.15)",
                zIndex: 100,
                animation: "fade-in-down 0.15s cubic-bezier(0.16, 1, 0.3, 1)"
              }}>
                {BOARDS.map((b) => (
                  <button
                    key={b.id}
                    onClick={async () => {
                      setProject((p: any) => ({ ...p, board: b.id }));
                      if (projectId) {
                        await updateDoc(doc(db, "projects", projectId), { board: b.id });
                      }
                      setShowBoardDropdown(false);
                    }}
                    style={{
                      background: currentBoard.id === b.id ? "rgba(157,39,222,0.2)" : "transparent",
                      border: "none",
                      borderRadius: 4,
                      padding: "6px 8px",
                      display: "flex",
                      ...
                      cursor: "pointer",
                      textAlign: "left",
                      color: currentBoard.id === b.id ? "#F2F2F0" : "rgba(242,242,240,0.6)",
                      transition: "all 0.1s"
                    }}
                    onMouseEnter={e => {
                      if (currentBoard.id !== b.id) e.currentTarget.style.background = "rgba(157,39,222,0.1)";
                    }}
                    onMouseLeave={e => {
                      if (currentBoard.id !== b.id) e.currentTarget.style.background = "transparent";
                    }}
                  >
                    <div style={{ width: 6, height: 6, borderRadius: "50%", background: b.color, flexShrink: 0, boxShadow: `0 0 6px ${b.color}` }} />
                    <div style={{ display: "flex", flexDirection: "column", flex: 1, minWidth: 0 }}>
                      <span style={{ fontSize: 11, fontWeight: currentBoard.id === b.id ? 600 : 500, fontFamily: "Space Grotesk, sans-serif", letterSpacing: "0.01em", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{b.name}</span>
                      <span style={{ fontSize: 9, color: "rgba(242,242,240,0.3)", marginTop: 2, fontFamily: "Space Grotesk, sans-serif", letterSpacing: "0.02em" }}>{b.fqbn.split(":")[0].toUpperCase()} PLATFORM</span>
                    </div>
                    {currentBoard.id === b.id && (
                      <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#A855F7" strokeWidth="3" style={{ flexShrink: 0 }}>
          ...
          <button
            ...
            style={{
              ...
              background: "transparent", border: "1px solid rgba(157,39,222,0.15)",
              borderRadius: 6, color: "rgba(242,242,240,0.55)",
              cursor: "pointer", transition: "all 0.15s",
              fontFamily: "Space Grotesk, sans-serif",
            }}
            onMouseEnter={e => { e.currentTarget.style.background = "rgba(157,39,222,0.1)"; e.currentTarget.style.color = "#F2F2F0"; e.currentTarget.style.borderColor = "rgba(157,39,222,0.35)"; }}
            onMouseLeave={e => { e.currentTarget.style.background = "transparent"; e.currentTarget.style.color = "rgba(242,242,240,0.55)"; e.currentTarget.style.borderColor = "rgba(157,39,222,0.15)"; }}
          >
          ...
          <button
            ...
            style={{
              display: "flex", alignItems: "center", gap: 5,
              padding: "5px 10px", fontSize: 11,
              background: connectedPort ? "rgba(74,222,128,0.08)" : "transparent",
              border: `1px solid ${connectedPort ? "rgba(74,222,128,0.3)" : "rgba(157,39,222,0.15)"}`,
              borderRadius: 6,
              color: connectedPort ? "#4ade80" : "rgba(242,242,240,0.55)",
              cursor: "pointer", transition: "all 0.15s",
              fontFamily: "Space Grotesk, sans-serif",
            }}
            onMouseEnter={e => {
              if (!connectedPort) {
                ...
                e.currentTarget.style.borderColor = "rgba(157,39,222,0.35)";
              }
            }}
            onMouseLeave={e => {
              if (!connectedPort) {
                e.currentTarget.style.background = "transparent";
                e.currentTarget.style.color = "rgba(242,242,240,0.55)";
                e.currentTarget.style.borderColor = "rgba(157,39,222,0.15)";
              }
            }}
          >
            <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
              {connectedPort ? (
                <><circle cx="12" cy="12" r="3" fill="currentColor" /><path d="M12 2v3M12 19v3M4.22 4.22l2.12 2.12M17.66 17.66l2.12 2.12M2 12h3M19 12h3M4.22 19.78l2.12-2.12M17.66 6.34l2.12-2.12" /></>
              ) : (
                <><path d="M8 8v8M16 8v8M4 12h16" /></>
              )}
            </svg>
            {connectedPort ? "Connected" : "Connect"}
          </button>

          {/* Separator */}
          <div style={{ width: 1, height: 24, background: "rgba(157,39,222,0.15)", margin: "0 4px" }} />
          ...
          <button
            ...
            style={{
              ...
              padding: "5px 10px", fontSize: 11,
              background: "transparent", border: "1px solid rgba(157,39,222,0.15)",
              borderRadius: 6, color: compileStatus === "success" ? "rgba(242,242,240,0.55)" : "rgba(242,242,240,0.2)",
              cursor: compileStatus === "success" ? "pointer" : "default", transition: "all 0.15s",
              fontFamily: "Space Grotesk, sans-serif",
            }}
            onMouseEnter={e => { if (compileStatus === "success") { e.currentTarget.style.background = "rgba(157,39,222,0.1)"; e.currentTarget.style.color = "#F2F2F0"; e.currentTarget.style.borderColor = "rgba(157,39,222,0.35)"; } }}
            onMouseLeave={e => { if (compileStatus === "success") { e.currentTarget.style.background = "transparent"; e.currentTarget.style.color = "rgba(242,242,240,0.55)"; e.currentTarget.style.borderColor = "rgba(157,39,222,0.15)"; } }}
          >
            <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
          ...
          <button
            ...
            title={!canCompile ? (compileBlockReason || "Compile limit reached") : "Compile project (generates firmware)"}
            style={{
              display: "flex", alignItems: "center", gap: 6,
              padding: "6px 14px", fontSize: 12, fontWeight: 600,
              background: compiling ? "rgba(157,39,222,0.15)" : "linear-gradient(135deg, #9D27DE, #B94FF0)",
              border: compiling ? "1px solid rgba(157,39,222,0.4)" : "none",
              borderRadius: 7, color: "#fff",
              cursor: compiling || !canCompile ? "not-allowed" : "pointer",
              opacity: !canCompile ? 0.5 : 1,
              transition: "all 0.2s",
              fontFamily: "Space Grotesk, sans-serif",
              boxShadow: compiling ? "none" : "0 2px 12px rgba(157,39,222,0.35)",
              letterSpacing: "0.01em",
            }}
            onMouseEnter={e => { if (!compiling && canCompile) e.currentTarget.style.boxShadow = "0 4px 20px rgba(157,39,222,0.55)"; }}
            onMouseLeave={e => { if (!compiling && canCompile) e.currentTarget.style.boxShadow = "0 2px 12px rgba(157,39,222,0.35)"; }}
          >
            {compiling ? (
              <>
          ...
          <button
            ...
            title={!firmwareReady ? "Compile first to flash" : "Flash firmware to device"}
            style={{
              display: "flex", alignItems: "center", gap: 6,
              padding: "6px 14px", fontSize: 12, fontWeight: 600,
              background: "transparent",
              border: `1px solid ${(compileStatus === "success" && firmwareReady) ? "rgba(74,222,128,0.5)" : "rgba(157,39,222,0.2)"}`,
              borderRadius: 7,
              color: (compileStatus === "success" && firmwareReady) ? "#4ade80" : "rgba(242,242,240,0.25)",
              cursor: (compileStatus !== "success" || !firmwareReady) ? "not-allowed" : "pointer",
              transition: "all 0.2s",
              fontFamily: "Space Grotesk, sans-serif",
              letterSpacing: "0.01em",
            }}
            onMouseEnter={e => { if (compileStatus === "success" && firmwareReady) { e.currentTarget.style.background = "rgba(74,222,128,0.08)"; e.currentTarget.style.boxShadow = "0 2px 12px rgba(74,222,128,0.2)"; } }}
            onMouseLeave={e => { if (compileStatus === "success" && firmwareReady) { e.currentTarget.style.background = "transparent"; e.currentTarget.style.boxShadow = "none"; } }}
          >
        ...
        {showPlanBanner && (
          <div style={{ width: "100%", borderTop: "1px solid rgba(157,39,222,0.12)" }}>
      ...
      {viewMode === "code" && (
        <CodeEditorView
          ...
          onChange={(newCode) => {
            ...
            // Mark that editor has diverged from blocks if sync is on
            if (syncEnabled && newCode !== generatedCode) {
              setCodeOverridesBlocks(true);
            }
          }}
          boardId={project?.board || "esp32-wroom"}
          projectName={project?.name || "sketch"}
          syncEnabled={syncEnabled}
          onToggleSync={handleToggleSync}
          syncedCode={generatedCode}
          codeOverridesBlocks={codeOverridesBlocks}
          onResetToBlocks={handleResetToBlocks}
          ...
          compileBlockReason={compileBlockReason ?? undefined}
        />
      )}
      ...
      <aside className="ide-panel" style={{
        background: "#0A0010",
        borderLeft: "1px solid rgba(157,39,222,0.12)",
        display: viewMode === "blocks" ? "flex" : "none"
      }}>
        {/* Panel tabs */}
        <div style={{
          display: "flex", borderBottom: "1px solid rgba(157,39,222,0.12)",
          padding: "0", background: "rgba(8,4,18,0.8)"
        }}>
          {([
            {
              id: "code", label: "Code",
          ...
          ] as { id: PanelTab; label: string; icon: any }[]).map((tab) => (
            <button
              key={tab.id}
              id={`panel-tab-${tab.id}`}
              ...
              style={{
                ...
                color: panelTab === tab.id ? "#B94FF0" : "rgba(242,242,240,0.35)",
              ...
              onMouseEnter={e => { if (panelTab !== tab.id) e.currentTarget.style.color = "rgba(242,242,240,0.65)"; }}
              onMouseLeave={e => { if (panelTab !== tab.id) e.currentTarget.style.color = "rgba(242,242,240,0.35)"; }}
            >
              {tab.icon}
              {tab.label}
            </button>
        ...
        <div style={{ flex: 1, overflow: "auto", padding: 0 }}>
          ...
          {panelTab === "info" && (
            <div style={{ padding: 16 }}>
              <h3 style={{ fontSize: 13, fontWeight: 700, color: "rgba(242,242,240,0.6)", marginBottom: 14, textTransform: "uppercase", letterSpacing: "0.08em", display: "flex", alignItems: "center", gap: 6 }}>
                <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="5" y="2" width="14" height="20" rx="2" /><path d="M9 6h6M9 10h6M9 14h4" /></svg>
                Board Details
              </h3>
      ...
      {/* ── Status Bar ───────────────────────────────────── */}
      <footer className="ide-statusbar" style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 5, marginRight: 14 }}>
          <span style={{
            ...
            background: compileStatus === "success" ? "#4ade80"
              : compileStatus === "error" ? "#f87171"
              : compileStatus === "compiling" ? "#B94FF0"
              : "rgba(157,39,222,0.3)",
            boxShadow: compileStatus === "success" ? "0 0 5px #4ade80"
              : compileStatus === "compiling" ? "0 0 5px #B94FF0"
              : "none",
          }} />
          <span style={{
            color: compileStatus === "success" ? "#4ade80"
              : compileStatus === "error" ? "#f87171"
              : compileStatus === "compiling" ? "#B94FF0"
              : "rgba(157,39,222,0.35)",
            letterSpacing: "0.05em",
          }}>
            {compileStatus === "success" ? "Build OK"
              : compileStatus === "error" ? "Build Failed"
              : compileStatus === "compiling" ? "Compiling…"
              : "Ready"}
          </span>
        </div>

        <div style={{ width: 1, height: 14, background: "rgba(157,39,222,0.1)", marginRight: 14 }} />

        {/* Board */}
        <div style={{ display: "flex", alignItems: "center", gap: 5, marginRight: 14 }}>
          <span style={{ width: 5, height: 5, borderRadius: "50%", background: currentBoard.color, display: "inline-block", boxShadow: `0 0 5px ${currentBoard.color}` }} />
          <span style={{ color: currentBoard.color, fontWeight: 500 }}>{currentBoard.name}</span>
        </div>
        ...
        <span style={{ letterSpacing: "0.04em" }}>
          {blocklyReady ? "Editor ready" : "Loading…"}
        </span>

        {/* WebSerial warning */}
        {!flashSupported && (
          <>
            <div style={{ width: 1, height: 14, background: "rgba(157,39,222,0.1)", margin: "0 14px" }} />
            <span style={{ color: "#F59E0B", display: "flex", alignItems: "center", gap: 4 }}>
              <svg width="9" height="9" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>
              Use Chrome/Edge for WebSerial
            </span>
          </>
        )}

        {/* Connected port */}
        {connectedPort && flashSupported && (
          <>
            <div style={{ width: 1, height: 14, background: "rgba(157,39,222,0.1)", margin: "0 14px" }} />
            <span style={{ color: "#4ade80", display: "flex", alignItems: "center", gap: 4 }}>
              <svg width="9" height="9" viewBox="0 0 24 24" fill="#4ade80"><circle cx="12" cy="12" r="5"/></svg>
              USB Connected
            </span>
          </>
        )}
        ...
        <span style={{ marginLeft: "auto", letterSpacing: "0.04em" }}>
          {saving ? (
            <span style={{ color: "#9D27DE" }}>Saving…</span>
          ) : (
            <span>Saved</span>
          )}
        </span>
